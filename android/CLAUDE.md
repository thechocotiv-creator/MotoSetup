...
3. **Google Sign-In**: serve registrare lo SHA-1 del debug keystore su Firebase Console (`keytool -list -v -keystore %USERPROFILE%\.android\debug.keystore -alias androiddebugkey -storepass android -keypass android`, keystore creato automaticamente al primo build). Provare prima l'automazione via Firebase CLI se disponibile.
