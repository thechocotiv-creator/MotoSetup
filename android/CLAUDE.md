...
Implementato in Fase 2 (`navigation/AppRoute.kt`, `AppSheet.kt`, `AppDialog.kt`, `PaywallReason.kt`, `AppNavActions.kt`, `RootScaffold.kt`). Le schermate di destinazione sono reali dalla Fase 6 in poi (Home/Setup/Manutenzione/Consigli AI/Profilo, relative sheet e dialog inclusi) — solo Play Billing (punto 4 sotto) resta fake fino alla Fase 7.
...
9. **Consigli AI — Cloud Function LLM rimandata**: come per Play Billing (punto 4), niente infrastruttura server in questa fase. `FirebaseAIAdviceRepository` (Fase 5) scrive/legge davvero `users/{uid}/aiAdvice` su Firestore e aggiorna `aiUsedToday`/`aiUsageDate` via transazione, ma il "consiglio" è generato localmente da `data/ai/MockAiAdvisor.kt` (euristica a parole chiave, nessuna chiamata di rete) invece che dalla Cloud Function LLM descritta nel CLAUDE.md radice. Quando la Cloud Function (Vertex AI in Firebase, scelta condivisa con iOS) sarà pronta, sostituire solo la chiamata a `generateMockAdvice` in `FirebaseAIAdviceRepository.askAdvice` con l'invocazione della function — nessun altro layer (model, repository interface, ViewModel, UI) dovrebbe cambiare.
